...
/*   Updated: 2025/09/12 17:56:51 by kosakats         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_x(t_linedraw *line, t_game *game)
{
	...
	{
		(line->step_x) = -1;
		(line->next_x_time) = ((game->player_pos_x) - (line->tile_x))
			* (line->delta_x);
	}
	else
	{
...
static void	check_y(t_linedraw *line, t_game *game)
{
	...
	{
		(line->step_y) = -1;
		(line->next_y_time) = ((game->player_pos_y) - (line->tile_y))
			* (line->delta_y);
	}
	else
	{
		(line->step_y) = 1;
		(line->next_y_time) = (((line->tile_y) + 1) - (game->player_pos_y))
			* (line->delta_y);
	}
}
...
void	init_line(t_linedraw *line, int X, t_game *game)
{
	...
	line->ray_x = game->player_dir_x + ((game->player_plane_x)
			* (line->camera_x));
	line->ray_y = game->player_dir_y + ((game->player_plane_y)
			* (line->camera_x));
